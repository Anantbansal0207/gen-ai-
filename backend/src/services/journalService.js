...
export class JournalService {
  ...
  static async getInitialPrompt(userId) {
    try {
      ...
      // If user has recent entries, customize the prompt based on patterns
      if (recentEntries && recentEntries.length > 0) {
        ...
        if (daysSinceLastEntry === 0) {
          return "Welcome back! I see you've been journaling today. How are you feeling now? Anything new you'd like to explore?";
        } else if (daysSinceLastEntry === 1) {
          return "Good to see you back after yesterday! How has your day been unfolding?";
        } else if (daysSinceLastEntry > 7) {
          return "Welcome back! It's been a while since your last entry. What's been happening in your life that you'd like to reflect on?";
        }
      }

      // Return a random prompt for new users or regular cases
