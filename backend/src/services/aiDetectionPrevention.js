...
async function refineWithGemini(text) {
  try {
    const model = genAI.getGenerativeModel({ model: 'gemini-1.5-flash' });
    const prompt = `You are a subtle editor for a therapy response.

Your task:
- Do NOT fix typos, spelling mistakes, or small imperfections — keep them as they are.
- Focus ONLY on improving the placement of filler words (such as "um", "like", "you know", "actually", etc.).
- Move filler words to more natural, smooth positions in the sentence without changing the meaning or tone.
- Preserve the original wording, tone, and content exactly, except for adjusting filler word placement.
- Return only the refined text with fillers repositioned naturally.
