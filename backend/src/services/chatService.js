...
async function refineResponse(originalResponse) {
  try {
      ...
      const prompt = `Please rewrite the following response from a therapy session to sound like it's coming directly from a human therapist.It shouldnt sound like u are an AI. Ensure it is empathetic, professional, and avoids any language that identifies it as being generated by an AI. Remove any phrases like "${AI_IDENTIFYING_KEYWORDS.join('", "')}" or similar self-referential statements.
